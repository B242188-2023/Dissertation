...
do
    ...
    samtools view -@ 160 -b -f 4 ${sample}_cds_sorted.bam > ${sample}_cds_unmapped_sorted.bam
    # bam -> fastq
    samtools fastq ${sample}_cds_unmapped_sorted.bam > ${sample}_cds_unmapped.fastq
    # de novo assembly
    ...
    # index
    # samtools index -@ 160 ${sample}_index ${sample}_cds_unmapped_sorted.bam

    # identify variations
    bcftools mpileup --threads 160 -Ou -f ../fastq/AeUmbellulata_TA1851_v1-cds.fasta\
     ${sample}_cds_denovo_sorted.bam | bcftools call -mv -o ${sample}_variants.vcf

    # filtering low quality variants
    bcftools view ${sample}_variants.vcf | bcftools filter --exclude 'QUAL < 10' \
    > ${sample}_filtered_variants.vcf

    bcftools view -v snps -m2 -M2 --min-ac 1:minor ${sample}_filtered_variants.vcf > ${sample}_snp.vcf
